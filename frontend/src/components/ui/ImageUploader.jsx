...
export default function ImageUploader({ 
  ...
  // Si el valor empieza por '/' (como /uploads/...), anteponemos la URL del backend si es necesario,
  // pero dado que el navegador lo resuelve localmente si corren bajo el mismo host, o Axios está configurado,
  // podemos obtener la URL base del backend desde la configuración de Axios o simplemente mostrarla.
  const backendBaseUrl = api.defaults.baseURL?.replace('/api', '') || 'http://localhost:8000';
  const displayUrl = value 
    ? (value.startsWith('http') ? value : `${backendBaseUrl}${value}`) 
    : '';
  ...
  const handleFileChange = async (e) => {
    ...
    // Validar tamaño de archivo (Max 4MB)
      ...
      <div className="relative group rounded-xl border border-border/80 bg-background/30 backdrop-blur-sm p-4 transition-all duration-300 hover:border-primary/50 flex flex-col sm:flex-row items-center gap-5">
        ...
        <div className="flex-1 flex flex-col gap-2 w-full text-center sm:text-left">
          <div>
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5">JPEG, PNG, WEBP o SVG hasta 4MB</p>
          </div>
