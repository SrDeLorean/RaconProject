...
export default function Datos() {
  ...
  // Reset activeSubPage if it becomes unavailable
  useEffect(() => {
    if (filterMode === 'equipo' && (activeSubPage === 'comparador' || activeSubPage === 'tiers')) {
      setActiveSubPage('resumen');
    }
  }, [filterMode, activeSubPage]);
        ...
        <div className="flex border border-border/40 p-1 bg-card/25 backdrop-blur-md rounded-2xl max-w-2xl mx-auto shadow-xl overflow-x-auto gap-1 mobile-scroll-indicator pb-2.5">
          {(() => {
            ...
            if (filterMode !== 'equipo') {
              list.push({ id: 'comparador', label: '⚔️ Comparador' });
              list.push({ id: 'tiers', label: '🏆 Tier List' });
            }
            return list;
          })().map((tab) => (
            <button 
          ...
          <div className="space-y-12">
            ...
            {activeSubPage === 'telemetria' && (
              <div className="animate-fade-in">
                <DirectorioTactico stats={filteredStats} getImageUrl={getImageUrl} filterMode={filterMode} />
              </div>
            )}
