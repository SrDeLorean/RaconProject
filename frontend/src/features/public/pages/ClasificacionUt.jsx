  ...
  if (standings.length === 0) {
    return (
      <p className="text-center text-xs text-muted-foreground italic py-8">
        {noResultsMsg || 'Sin participantes registrados.'}
      </p>
    );
  }
              ...
              >
                <td className="px-4 py-3 text-center">
                  <span className={`inline-flex items-center justify-center w-6 h-6 rounded font-black font-mono text-[10px] shadow-sm ${
                    idx === 0 && hasResults ? 'bg-amber-500 text-amber-950' :
                    idx === 1 && hasResults ? 'bg-slate-300 text-slate-900' :
                    idx === 2 && hasResults ? 'bg-amber-700/80 text-amber-100' :
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <span className="font-display font-black uppercase text-foreground truncate">
                      {s.nombre}
                    </span>
                  </div>
                </td>
                <td className="px-4 py-3 text-center font-mono">{s.pj}</td>
                <td className="hidden sm:table-cell px-4 py-3 text-center text-emerald-400 font-mono">{s.pg}</td>
                <td className="hidden sm:table-cell px-4 py-3 text-center font-mono">{s.pe}</td>
                <td className="hidden sm:table-cell px-4 py-3 text-center text-destructive font-mono">{s.pp}</td>
                ...
                <td className="px-4 py-3 text-center text-primary font-bold font-mono text-sm">{s.pts}</td>
                <td className="hidden lg:table-cell px-4 py-3">
                  <div className="flex items-center justify-center gap-1">
                    {s.ultimosCinco.length > 0 ? (
                      s.ultimosCinco.map((res, i) => {
                        const bgColor = res === 'v' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                                        res === 'x' ? 'bg-destructive/10 text-destructive border border-destructive/20' :
    ...
    <div className="space-y-4">
      ...
      {!hasResults && standings.length > 0 && (
        <div className="flex items-center gap-2 px-4 py-3 rounded-xl border border-amber-500/30 bg-amber-500/5 text-xs text-amber-400 font-bold">
          ⏳ El torneo aún no ha comenzado — se muestran los participantes inscritos en espera de resultados.
        </div>
      )}
      ...
      <div className="border border-border/50 bg-card/20 backdrop-blur-md rounded-2xl overflow-hidden shadow-lg">
        <StandingsTable
          standings={standings}
          noResultsMsg="No hay participantes inscritos en esta liga aún."
        />
      </div>
    </div>
  );
            ...
            {grupoKeys.map(grupo => {
              ...
              return (
                <div key={grupo} className="border border-border/50 bg-card/20 backdrop-blur-md rounded-2xl overflow-hidden shadow-lg">
                  ...
                  <StandingsTable
                    standings={standings}
                    noResultsMsg="Sin participantes en este grupo."
                  />
                </div>
              );
            })}
...
export default function ClasificacionUt() {
  ...
  const competencias = useMemo(() => {
    return competenciasList.filter(c => {
      ...
      count: partidos.filter(p => p.competencia_ut_id === c.id || p.competencia?.id === c.id).length
    }));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-12 lg:gap-16 items-center relative z-10">
          <div className="lg:col-span-7 relative flex flex-col items-start gap-4">
            ...
            <p className="animate-fade-in-up text-xs md:text-sm text-muted-foreground font-sans max-w-xl leading-relaxed tracking-wide font-light z-10 mt-2" style={{ animationDelay: '0.2s' }}>
              Clasificaciones en tiempo real de Ultimate Team (UT) 1v1 y 2v2 de todas las confederaciones. Ligas con tablas completas, fase de grupos de Copas y cuadros de Playoffs.
            </p>
          </div>
          ...
          <div className="lg:col-span-3 flex flex-col justify-center items-start lg:items-stretch gap-6 z-10 animate-fade-in-up" style={{ animationDelay: '0.15s' }}>
            <div className="border border-border/40 bg-card/45 backdrop-blur-md rounded-2xl p-6 w-full space-y-4 shadow-xl">
              <div className="flex justify-between items-center border-b border-border/30 pb-3">
                <span className="text-[10px] font-condensed tracking-widest text-muted-foreground uppercase">TELEMETRÍA UT</span>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <h4 className="text-[9px] font-condensed text-muted-foreground uppercase tracking-widest leading-none">TORNEOS UT</h4>
              ...
              <p className="text-[11px] text-muted-foreground leading-relaxed font-sans font-light">
                Tabla ponderada por enfrentamientos UT finalizados y reportados en el sistema de Torneos Pro FC.
              </p>
          ...
          <div className="space-y-8">
            <div className="filter-panel space-y-5">
              {totalJugados > 0 && (
                <div className="hidden sm:grid grid-cols-2 sm:grid-cols-4 gap-3 pb-4 border-b border-border/30">
                  <StatCard icon="🥇" label="Líder UT"     value={globalLeader?.nombre}  sub={globalLeader  ? `${globalLeader.pts} pts` : null} />
                  <StatCard icon="⚔️"  label="Mejor Ofensiva"    value={globalBestAtk?.nombre} sub={globalBestAtk ? `${globalBestAtk.gf} goles` : null} />
                  <StatCard icon="🛡️" label="Mejor Defensa"   value={globalBestDef?.nombre} sub={globalBestDef ? `${globalBestDef.gc} recibidos` : null} />
                  <StatCard icon="🎮" label="Partidos UT" value={String(totalJugados)}  sub={`en ${competenciasConPartidos.length} torneo${competenciasConPartidos.length !== 1 ? 's' : ''}`} />
                </div>
              )}
              ...
              {competencias.length > 0 && (
                <div className="space-y-2">
                  <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Torneo UT</p>
                  <div className="flex flex-wrap gap-2">
                    <FilterChip label="Todos" active={compFiltro === null}
              ...
              {activeFiltersCount > 0 && (
                <div className="flex items-center justify-between border-t border-border/30 pt-3">
                  <span className="text-[10px] text-muted-foreground">
                    Mostrando <span className="text-foreground font-bold">{competenciasConPartidos.length}</span> torneo(s) UT
                  </span>
                  <button
                    onClick={() => { setOrgFiltro(null); setCompFiltro(null); }}
            ...
            {competenciasConPartidos.length === 0 ? (
              <div className="text-center py-20 space-y-3">
                <div className="text-5xl">📊</div>
                <p className="text-base font-black uppercase tracking-wider text-foreground">Sin datos de partidos UT</p>
                <p className="text-sm text-muted-foreground">
                  Aún no hay fixtures de UT cargados. El organizador debe generar el calendario para ver las posiciones.
                </p>
              </div>
            ) : (
